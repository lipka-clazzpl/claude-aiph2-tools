...
LEARNING_DIR = Path(os.environ.get("AIPH2_LEARNING_DIR") or Path(__file__).resolve().parents[2] / "learning")
        ...
        rows.append(
            {
                ...
                "sm2": c.sm2.to_dict(),
                "path": str(c.path.relative_to(LEARNING_DIR.parent)) if c.path.is_absolute() and LEARNING_DIR.parent in c.path.parents else str(c.path),
            }
        )
