...
| Drążenie tematu — "powiedz więcej", "ciekawe", "a co jeśli..." | bieżący koncept | `record_interest(topic, '+lingered', -5)` | -5 |
...
| Ręczne nadpisanie przez `/interest <topic> <priority>` | wskazany | `record_interest(topic, '+manual', delta-do-celu)` | wg celu |
| Użytkownik wybrał gałąź Wikipedii (Krok 2f review) | temat wybranej gałęzi | `record_interest(topic, '+wiki-branch', -10)` | -10 |

## Zasady stosowania

- **Topic** to **slug** (małe litery z myślnikami), np. `customer-curiosity`, `lovable-plan-mode`, `super-loop`, `5-cech-buildera`. Nie pełne zdania.
- Sygnały są **kumulatywne** — pięć ponownych pytań w sesji oznacza pięć wywołań `record_interest`, każde z `weight_delta=-5`. Każde wywołanie dopisuje do listy `signals` w profilu.
- Priorytet jest **przycinany** do zakresu 0-100. Jeśli priorytet już jest 0, kolejny `-5` nic nie zmieni (informacja zostaje w `signals`).
...
| Przedział priorytetu | Głębokość kart pisanych w tej sesji |
|---|---|
...
Profil odczytujesz **na starcie sesji** (`read_interest_profile`). Aktualizujesz **w trakcie** (`record_interest`). Dla każdego nowego konceptu sprawdzasz, w którym przedziale jest jego temat, i dobierasz głębokość karty zapisywanej przez `add_card_full`.
...
Użytkownik: "Powiedz mi więcej o Customer Curiosity, ostatnio czytałem Teresa Torres"
   ...
   → (priorytet z 50 → 35; karta będzie standardowa, ale na granicy rozbudowanej)
...
Użytkownik (po 2 dniach, w nowej sesji): "Dobra, wracam do Customer Curiosity, mam pytanie"
   ...
   → (priorytet z 35 → 25; teraz przedział 0-30, karty rozbudowane)
```
